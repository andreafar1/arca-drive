...
public final class PhotoBackupWorker extends Worker {
    ...
    static void schedule(Context context) {
        ...
        Constraints constraints = new Constraints.Builder()
            ...
            .build();
        PeriodicWorkRequest work = new PeriodicWorkRequest.Builder(PhotoBackupWorker.class, 6, TimeUnit.HOURS)
    ...
    @NonNull @Override public Result doWork() {
        ...
        try {
            ...
            String destination = api.ensurePhoneBackupFolder();
            int uploaded = scanAndUpload(Uri.parse(treeValue), destination, api, settings);
    ...
    private int scanAndUpload(Uri treeUri, String destination, ApiClient api, SharedPreferences settings) throws Exception {
        ...
        while (!directories.isEmpty() && uploaded < MAX_FILES_PER_RUN && !isStopped()) {
            ...
            try (Cursor cursor = resolver.query(children, projection, null, null, null)) {
                ...
                while (cursor.moveToNext() && uploaded < MAX_FILES_PER_RUN && !isStopped()) {
                    ...
                    api.upload(resolver, document, name == null || name.isBlank() ? "foto-" + System.currentTimeMillis() : name, mime, destination, sent -> {});
